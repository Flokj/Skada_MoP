...
Skada:RegisterModule("Healing", function(L, P)
	...
	local function log_heal(set)
		...
		local actor = Skada:GetActor(set, heal.actorname, heal.actorid, heal.actorflags, heal.spell)
...
Skada:RegisterModule("Overhealing", function(L)
	...
	function mode_target_spell:Update(win, set)
		...
		for spellid, spell in pairs(spells) do
			...
			local d = win:spell(nr, spellid, true)
			d.value = spell.o_amt
			fmt_valuetext(d, mode.metadata.columns, spell.amount + d.value, actortime and (d.value / actortime), win.metadata, true)
		end
	end
