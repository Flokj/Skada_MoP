...
local GetAddOnMetadata = _G.C_AddOns and _G.C_AddOns.GetAddOnMetadata or GetAddOnMetadata
ns.author = GetAddOnMetadata(folder, "Author")
ns.version = GetAddOnMetadata(folder, "Version")
ns.date = GetAddOnMetadata(folder, "X-Date")
ns.website = "https://github.com/bkader/Skada-Damage-Meter"
...
-- locale utils
do
	local GAME_LOCALE = ns.GAME_LOCALE or GetLocale()
	ns.GAME_LOCALE = GAME_LOCALE
	ns.isEastAsia = GAME_LOCALE == "zhCN" or GAME_LOCALE == "zhTW" or GAME_LOCALE == "koKR"
end
...
do
	...
	local function IsWarWithin()
		return (WowBuild >= 110000 and WowBuild < 120000)
	end

	local function IsTimewalk()
		return (WowBuild < 50000)
	end
	...
	Private.IsWarWithin = IsWarWithin
...
function Private.RegisterClasses()
	...
	-- arena class colors
	classcolors.ARENA_GOLD = {r = 1, g = 0.82, b = 0}
	classcolors.ARENA_GREEN = {r = 0.1, g = 1, b = 0.1}
	...
	for class, info in pairs(classcolors) do
		info.colorStr = info.colorStr or RGBPercToHex(info.r, info.g, info.b, true)
	end
...
function Private.RegisterMedias()
	...
	-- sounds
	LSM:Register("sound", "Cartoon FX", 566543) -- [[Sound\Doodad\Goblin_Lottery_Open03.wav]]
	LSM:Register("sound", "Cheer", 567283) -- [[Sound\Event Sounds\OgreEventCheerUnique.wav]]
	LSM:Register("sound", "Explosion", 566982) -- [[Sound\Doodad\Hellfire_Raid_FX_Explosion05.wav]]
	LSM:Register("sound", "Fel Nova", 568582) -- [[Sound\Spells\SeepingGaseous_Fel_Nova.wav]]
	LSM:Register("sound", "Fel Portal", 569215) -- [[Sound\Spells\Sunwell_Fel_PortalStand.wav]]
	LSM:Register("sound", "Humm", 569518) -- [[Sound\Spells\SimonGame_Visual_GameStart.wav]]
	LSM:Register("sound", "Rubber Ducky", 566121) -- [[Sound\Doodad\Goblin_Lottery_Open01.wav]]
	LSM:Register("sound", "Shing!", 566240) -- [[Sound\Doodad\PortcullisActive_Closed.wav]]
	LSM:Register("sound", "Short Circuit", 568975) -- [[Sound\Spells\SimonGame_Visual_BadPress.wav]]
	LSM:Register("sound", "Simon Chime", 566076) -- [[Sound\Doodad\SimonGame_LargeBlueTree.wav]]
	LSM:Register("sound", "War Drums", 567275) -- [[Sound\Event Sounds\Event_wardrum_ogre.wav]]
	LSM:Register("sound", "Wham!", 566946) -- [[Sound\Doodad\PVP_Lordaeron_Door_Open.wav]]
	LSM:Register("sound", "You Will Die!", 546633) -- [[Sound\Creature\CThun\CThunYouWillDIe.wav]]
...
do
	...
	if ns.WowBuild >= 30000 and ns.WowBuild < 70000 then
		...
		-- other resources
		customSpells[9999001] = {L["Combo Points"], [[Interface\ICONS\inv_mace_2h_pvp410_c_01]]} --> Combo Points
		customSpells[9999002] = {L["Runes"], [[Interface\ICONS\spell_deathknight_frozenruneweapon]]} --> Runes
		customSpells[9999003] = {L["Soul Shards"], [[Interface\ICONS\inv_misc_gem_amethyst_02]]} --> Soul Shards
		customSpells[9999004] = {L["Astral Power"], [[Interface\ICONS\ability_druid_eclipseorange]]} --> Astral Power
		customSpells[9999005] = {L["Holy Power"], [[Interface\ICONS\achievement_bg_winsoa]]} --> Holy Power
		customSpells[9999006] = {L["Maelstrom"], [[Interface\ICONS\spell_fire_masterofelements]]} --> Maelstrom
		customSpells[9999007] = {L["Chi"], [[Interface\ICONS\ability_monk_healthsphere]]} --> Chi
		customSpells[9999008] = {L["Insanity"], [[Interface\ICONS\spell_priest_shadoworbs]]} --> Insanity
		customSpells[9999009] = {L["Arcane Charges"], [[Interface\ICONS\spell_arcane_arcane01]]} --> Arcane Charges
		customSpells[9999010] = {L["Fury"], [[Interface\ICONS\ability_demonhunter_eyebeam]]} --> Fury
		customSpells[9999011] = {L["Pain"], [[Interface\ICONS\ability_demonhunter_demonspikes]]} --> Pain
	end
	...
	if C_Spell and C_Spell.GetSpellInfo then
		Private.SpellInfo = function(spellid)
			spellid = spellid and math_abs(SpellSplit(spellid)) or nil
			if not spellid then return end

			local name, rank, icon, castTime, minRange, maxRange, spellID, originalIcon
			if customSpells[spellid] then -- a custom spell?
				name, icon = customSpells[spellid][1], customSpells[spellid][2]
				return name, nil, icon
			end

			local info = C_Spell.GetSpellInfo(spellid)
			if info and info.name then
				info.name = spellid == 6603 and L["Melee"] or info.name
				return info.name, nil, info.icon or info.iconID, info.castTime, info.minRange, info.maxRange, info.spellID, info.originalIcon
			end
		end
	elseif GetSpellInfo then
		Private.SpellInfo = function(spellid)
			...
			name = spellid == 6603 and L["Melee"] or name
			icon = customIcons[spellid] or icon
			return name, rank, icon, castTime, minRange, maxRange, spellID, originalIcon
		end
	else
		Private.SpellInfo = _G.Multibar_EmptyFunc
	end
...
do
	...
	ns.userRealm = gsub(GetRealmName(), ns.WowBuild >= 100200 and "[%s-]" or "%s", "")
	...
	local function UnitFullname(unit, ownerUnit, fmt)
		ownerUnit = ownerUnit ~= unit and ownerUnit or nil
	...
	if ns.WowBuild >= 100200 then
		...
		UnitFullname = function(unit, ownerUnit, fmt)
			...
			name = name or L["Unknown"]
			return ownerUnit and (name or L["Unknown"]) or format("%s-%s", name or L["Unknown"], realm or realm_name)
		end
	end
...
do
	...
	function setPrototype:Bind(obj)
		...
		self.arena = (obj and obj.type == "arena")
		return obj
	end
...
do
	...
	function Window:actor(d, actor, is_enemy, actorname)
		if d and actor then
			...
			-- actor's bar needs to be refreshed?
			if actor.__mod then
				actor.__mod = nil
				d.refresh = true
			end
