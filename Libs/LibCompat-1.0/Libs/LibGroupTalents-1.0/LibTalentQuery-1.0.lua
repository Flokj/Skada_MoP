...
local MAJOR, MINOR = "LibTalentQuery-1.0", 90000 + tonumber(("$Rev: 87 $"):match("(%d+)"))
...
local frame = lib.frame or CreateFrame("Frame", MAJOR .. "_Frame")
lib.frame = frame
...
local function GuidToUnitID(guid)
	...
	for i = 0, num + 3 do
		local unit = i == 0 and "player" or i == num + 1 and "target" or i == num + 2 and "focus" or i == num + 3 and "mouseover" or prefix .. i

		local target = unit .. "target"
		if UnitGUID(target) == guid then
			return target
		elseif i <= num then
			target = unit .. "pettarget"
			if UnitGUID(target) == guid then
				return target
			end
		end
...
function lib:Query(unit)
	if UnitLevel(unit) < 10 or UnitName(unit) == UNKNOWN then return end
...
function lib:CheckInspectQueue()
	...
	if self.lastInspectPending > 0 or not enteredWorld then return end
...
function lib:NotifyInspect(unit)
	if not (UnitExists(unit) and UnitIsVisible(unit) and UnitIsConnected(unit)) then return end
	...
	_, self.lastInspectTree = GetTalentTabInfo(1, isnotplayer) -- Talent tree names are available immediately
end
...
function lib:INSPECT_READY()
	...
	if self.lastInspectName and self.lastInspectPending == 0 then
		...
		if UnitGUID(self.lastInspectUnit) == self.lastInspectGUID then
			...
			local _, tree1, _, _, spent1 = GetTalentTabInfo(1, isnotplayer, nil, group)
			...
			local _, tree2, _, _, spent2 = GetTalentTabInfo(2, isnotplayer, nil, group)
			local _, tree3, _, _, spent3 = GetTalentTabInfo(3, isnotplayer, nil, group)
