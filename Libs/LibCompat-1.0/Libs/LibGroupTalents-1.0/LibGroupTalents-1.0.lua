...
local tonumber, format, strsplit = tonumber, string.format, string.split
local tinsert, tremove, tconcat, unpack = table.insert, table.remove, table.concat, unpack
local UnitClass, UnitGUID, UnitName, UnitLevel = _G.UnitClass, _G.UnitGUID, _G.UnitName, _G.UnitLevel
local UnitExists, UnitIsUnit, UnitInParty, UnitInRaid = _G.UnitExists, _G.UnitIsUnit, _G.UnitInParty, _G.UnitInRaid
local UnitIsConnected, UnitIsVisible = _G.UnitIsConnected, _G.UnitIsVisible
local CanInspect = _G.CanInspect
local GetSpellInfo, GetTime = _G.GetSpellInfo, _G.GetTime
local GetTalentInfo, GetTalentTabInfo = _G.GetTalentInfo, _G.GetTalentTabInfo
local GetNumTalents, GetUnspentTalentPoints = _G.GetNumTalents, _G.GetUnspentTalentPoints
local GetActiveSpecGroup = _G.GetActiveSpecGroup or _G.GetActiveTalentGroup
local crc32 -- defined later
...
local specChangers = {}
for index, spellid in pairs(_G.TALENT_ACTIVATION_SPELLS) do
...
local next, select, pairs, type, wipe = next, select, pairs, type, wipe
			...
			if delay > 1 then
				...
				for guid, when in pairs(lib.talentTimers) do
					if now > when then
						...
						triggers = triggers or new()
						triggers[guid] = true
						lib.talentTimers[guid] = nil
						if next(lib.talentTimers) == nil then
...
local function AnyPending()
	...
	for guid, info in pairs(lib.roster) do
		...
		if UnitIsConnected(namerealm) then
			...
			lib.wasOffline = lib.wasOffline or new()
...
function lib:UNIT_AURA(unit)
	...
	if r and (not UnitIsVisible(unit) or (self.wasOffline and self.wasOffline[guid])) then
		self.outOfSight = self.outOfSight or {}
	...
	if next(subtractions) then
		for guid in pairs(subtractions) do
			if self.outOfSight then
				...
				if next(self.outOfSight) == nil then
					self.outOfSight = del(self.outOfSight)
				end
			end
			...
			if r then
				...
				if classStorageStrings then
					...
					if next(classStorageStrings) == nil then
...
do
	...
	crc32 = function(str)
		local val = tonumber(build) -- Use WoW build as CRC base
		for i = 1, #str do
		...
		if r then
			...
			if r.talents and r.active and not r.realm and (not r.unspent or not r.unspent[r.active]) then
				...
				for i = 1, r.numActive do
					local t = r.talents[i]
					if t then
						str = format("%s;%d,%s", str, i, tconcat(t, "-"))
					end
				end
	...
	function lib:SetStorageString(str, comms)
		...
		if str then
			...
			if #parts >= 2 then
				...
				local temp = tconcat(parts, ";", 1, #parts - 1)
...
--@end-debug@]===]
		...
		for tab = 1, numTabs do
			...
			tinsert(n, tconcat(t, nil, 1, trim or 0))
			del(t)
		end
...
function lib:TalentQuery_Ready(e, name, realm, unit)
	...
	if r then
		...
		if GetTalentTabInfo(1, isnotplayer) then
			...
			local listUnspent = lib:GetUnspentTalentPoints(unit, active)
			local invalid = nil
			local talents = new()
...
function lib:RefreshPlayerGlyphs()
	...
	for talentGroup = 1, GetNumTalentGroups() do
		...
		glyphs[talentGroup] = tconcat(list, ",")
		del(list)
	end
...
function lib:TriggerRefreshTalents(guid, delay)
	if guid then
		self.talentTimers = self.talentTimers or new()
...
function lib:RefreshTalentsByGUID(guid)
	local r = self.roster[guid]
	if not r then return end
	if not ValidateUnit(r, guid) then return end
	...
	self.talentThrottle = self.talentThrottle or {}
...
do
	...
	function lib:GetGUIDRole(guid, reset)
		local r = guid and self.roster[guid]
		if not r then return end
		if r.role and not reset then return r.role end
		if not ValidateUnit(r, guid) then return end
			...
			if class == "DEATHKNIGHT" then
				...
				elseif class == "WARRIOR" then
					...
					elseif class == "DRUID" then
						if heavy == 2 then
							role = self:GUIDHasTalent(guid, survivalOfTheFittest) and self:GUIDHasTalent(guid, protectorOfThePack) and "tank" or "melee"
						else
...
local function CanCommQuery(guid)
	if not lib.commQueried or not lib.commQueried[guid] then
		lib.commQueried = lib.commQueried or new()
...
function lib:GetTalentTabInfo(unit, tab, group)
	if UnitIsUnit(unit, "player") then
		return GetTalentTabInfo(tab, nil, nil, group or GetActiveSpecGroup())
	else
		local guid = unit and UnitGUID(unit)
		local r = guid and self.roster[guid]
...
function lib:GetTalentInfo(unit, tab, index, group)
	if UnitIsUnit(unit, "player") then
		return GetTalentInfo(tab, index, nil, nil, group or GetActiveSpecGroup())
	else
		local _, class = UnitClass(unit)
		if class then
			local ctd = self.classTalentData[class]
...
do
	...
	function lib:IterateRoster()
		...
		if GetNumGroupMembers() > 0 then
			...
			t.mode = "party"
			t.id = 0
			t.p = GetNumSubgroupMembers()
		end
		return iter, t
	end
end

-- Cataclysm modifications
local _, _, _, WowBuild = GetBuildInfo()
if WowBuild < 40000 or WowBuild >= 50000 then return end
...
do
	...
	local GetMinorTalentTreeBonuses = _G.GetMinorTalentTreeBonuses
	local GetMajorTalentTreeBonuses = _G.GetMajorTalentTreeBonuses
	...
	function lib:GetMinorTalentTreeBonuses(unit, tab)
		...
		return tab and getTalentTreeBonuses("minor", unit, tab) or nil
	end
	...
	function lib:GetMajorTalentTreeBonuses(unit, tab)
		...
		return tab and getTalentTreeBonuses("major", unit, tab) or nil
	end
	...
	-- GetClassTalentData
	-- Builds an internal table for talent name -> tree/index lookups.
	GetClassTalentData = function(unit)
		local _, class = UnitClass(unit)
		if class then
			local data = lib.classTalentData[class]
			if not data then
...
-- GetTreeNames
lib.GetTreeNames = function(self, class)
	local info = self.classTalentData[class]
...
do
	...
	lib.TalentQuery_Ready = function(self, e, name, realm, unit)
		GetClassTalentData(unit)
...
-- GetGUIDGlyphs
lib.GetGUIDGlyphs = function(self, guid, group)
	local r = self.roster[guid]
	if r then
		local glyphs = r.glyphs and r.glyphs[group or r.active]
		if glyphs then
			local temp = new(strsplit(",", glyphs))
...
-- RefreshPlayerGlyphs
lib.RefreshPlayerGlyphs = function(self)
	local guid = UnitGUID("player")
	local r = self.roster[guid]
	if not r then return end
...
-- SetStorageString
lib.SetStorageString = function(self, str, comms)
	local ret, retInfo
	if str then
		local parts = new(strsplit(";", str))
		if #parts >= 2 then
...
-- OnReceiveTalents
lib.OnReceiveTalents = function(self, guid, unit, talents, primary, active, numActive, listUnspent)
	local r = self.roster[guid]
	if r then
		if active ~= r.active or numActive ~= r.numActive or not CompareTalents(talents and talents[active], r.talents and r.talents[r.active]) then
			local oldTalents
...
do
	...
	-- GetGUIDRole
	lib.GetGUIDRole = function(self, guid, reset)
		local r = guid and self.roster[guid]
		if not r then
			return
		end
		...
		elseif r.talents and r.talents[r.active] then
			...
			if (t1 + t2 + t3) > 0 then
				...
				elseif class == "DEATHKNIGHT" then
					...
					elseif class == "DRUID" then
						if heavy == 2 then
							role = self:GUIDHasTalent(guid, naturalReaction) and self:GUIDHasTalent(guid, survivalInstincts) and self:GUIDHasTalent(guid, thickHide) and "tank" or "melee"
						else
							role = heavy == 1 and "caster" or "healer"
						end
					elseif class == "SHAMAN" then
...
-- GUIDHasTalent
-- Returns: nil, or number of points spent into talent
lib.GUIDHasTalent = function(self, guid, talentName, group)
	local talents, r = GetGUIDTalentsRaw(guid, group)
	if talents and r.class then
		local data = self.classTalentData[r.class]
		if data then
...
-- GetClassTalentInfo
lib.GetClassTalentInfo = function(self, class, talentName)
-- Returns: Max Rank, Icon, Tab, Tier, Column, Tree Index
	local data = self.classTalentData[class]
